...
int index_save(const Index *index) {
    ...
    // Sort by path (insertion sort)
    for (int i = 1; i < count; i++) {
        const IndexEntry *key = sorted[i];
        int j = i - 1;
        while (j >= 0 && strcmp(sorted[j]->path, key->path) > 0) {
            sorted[j + 1] = sorted[j];
            j--;
        }
        sorted[j + 1] = key;
...
int index_add(Index *index, const char *path) {
    ...
    void *buf = malloc((size_t)(fsize > 0 ? fsize : 1));
